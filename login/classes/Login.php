...
class Login
{
    ...
    /**
     * @var boolean $user_is_admin The user's login status
     */
    ...
    private function loginWithSessionData()
    {
        ...
        $this->user_is_admin = $_SESSION['user_admin'];
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email, user_admin FROM users WHERE user_id = :user_id
                                                      AND user_rememberme_token = :user_rememberme_token AND user_rememberme_token IS NOT NULL");
                    $sth->bindValue(':user_id', $user_id, PDO::PARAM_INT);
                    $sth->bindValue(':user_rememberme_token', $token, PDO::PARAM_STR);
                    $sth->execute();
                    // get result row (joyas an object)
                    ...
                    if (isset($result_row->user_id)) {
                        ...
                        $this->user_is_admin = $result_row->user_admin;
                        $this->user_is_logged_in = true;
    ...
    /**
     * Simply return the current state of the user's admin privileges
     * @return bool user's login status
     */
