<?php
    ...
    $brMOC = new mocBusinessRules($daoMethodOfCapture);

class mocBusinessRules
{ 

    protected $pageRowCount = 0;      // How many rows to display on the web page.
    protected $prevPageRowCount = 0;  // The previous row display amount.
    protected $recPointer = 0;        // The last record displayed on the web page.
    protected $queryRecCount = 0;     // The actual amount of records returned from the query.
    protected $totalMOCRecords = 0;   // The total records in the Method of Capture table.
    protected $action = "";           // What action was invoked on the web page.
    protected $totalPages = 0;        // The total number of pages based on the row count and total records.
    protected $daoMOC;                // The dao object for the data.

    public function __construct(MethodOfCapture $daoMOC)
    {

        $this->daoMOC = $daoMOC;
        $pageRowCount = $this->initPageRowCount();
        $totalMOCRecords = $this->setTotalMOCRecords();
        $prevPageRowCount = $this->initPrevPageRowCount();
        $recPointer = $this->initRecordPointer();   // Determines what the start record should be based on the web page action.
        $action = $this->setAction();
        $totalPages = ceil($totalMOCRecords / $pageRowCount);
